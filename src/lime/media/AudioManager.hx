...
class AudioManager
{
	...
	**/
	...
	/**
		...
		Only works on Native target.
		BUG: In SDL3 backend (All platforms except IOS), it will return "Default Device" instead of
			the actual default device.
	...
	**/
	...
	@:noCompletion private static function onActivate():Void
	{
		if (__resumeOnFocus)
		{
			__resumeOnFocus = false;
			AudioManager.resume();
		}
	}

	@:allow(lime._internal.backend.native.NativeApplication)
	@:noCompletion private static function onDeactivate():Void
	{
